...
-- Defaults for re-execution when _G._AdaptK7Extras came from an older build.
if AdaptK7Extras.antiDieDeathConns==nil then AdaptK7Extras.antiDieDeathConns={} end
-- One-time cleanup for older Spirit Hub builds that had Anti Drop enabled.
do
...
-- ============================================================
-- ADAPT COOLDOWN ESP VISUAL (visual only; does not change cooldown logic)
-- Removes the heavy card/brackets from numeric cooldown BillboardGuis
-- attached to the local character and keeps only Adapt-style text.
-- ============================================================
...
_G._AdaptStyleCooldownVisuals = function(char)
    ...
    local function styleRoot(root)
        ...
        for _, bb in ipairs(root:GetDescendants()) do
            if bb:IsA("BillboardGui") and bb.Name ~= "AdaptBillboard" then
...
do
    ...
    -- Visual only: Adapt-style cooldown ESP. The timer/ragdoll logic below is unchanged.
    local function createTimerBillboard()
        ...
        -- Adapt-style cooldown: compact, transparent and text-only.
        timerBillboard.Size = UDim2.new(0, 92, 0, 26)
    ...
    local function updateTimerDisplay(time, isReady)
        ...
        -- Keep only the Adapt visual here; no cooldown/timer behavior is changed.
        timerLabel.TextColor3 = Color3.fromRGB(255,255,255)
        timerLabel.TextStrokeColor3 = Color3.fromRGB(0,0,0)
        timerLabel.TextStrokeTransparency = 0
        if isReady then
...
-- Manual mode: one extra jump per jump-button press (PC + mobile).
-- Hold mode keeps its own Heartbeat loop in startHoldInfJump().
...
-- ============================================================
-- ZEY AUTO STEAL ENGINE (Normal / Normal V2 / Semi)
-- ============================================================
...
-- ============================================================
-- AUTO STEAL (Zey source: Normal / V2 / Semi)
-- ============================================================
...
local function armDrop()
    -- Ignore delayed/overlapping Drop input immediately after TP Down.
    if tick() < (_dropSuppressedUntil or 0) then return nil end
    _dropToken = (_dropToken % 1000000) + 1
    return _dropToken
end
local function cancelDropForTPDown()
    -- TP Down is a separate action: never allow a pending/active Drop to continue into it.
    -- Keep a short guard because Roblox UI/input can deliver a second Activated/InputBegan a few frames later.
...
runDrop=function(expectedToken)
    -- Never let a delayed Drop event fire as part of TP Down.
...
-- ============================================================
-- ADAPT VISUAL / DEFENSE EXTRAS (ported from supplied 7UP source)
-- ============================================================
...
-- Hard Hit from the supplied source.
...
-- Anti Die logic supplied by the user: infinite health + Dead state disabled.
function AdaptK7Extras.clearAntiDieConnections()
    -- Clean connections from both this build and the previous Anti Die implementation.
...
-- Preserve the previous default: Anti Fling starts enabled, now it has a visible toggle.
...
-- ============================================================
-- PLAYER ESP SPEED + BOX + TRACERS (theme color) + K7 user marker
-- ============================================================
...
local function updatePlayerEsp(plr)
    ...
    -- ESP Speed: shows the target's horizontal movement speed above the head.
    if playerEspEnabled then
        ...
        -- clean old highlight objects from previous builds
        if o.highlight then pcall(function() o.highlight:Destroy() end); o.highlight = nil end
    end

    -- ESP Box: 2D box around each player. Falls back to an outline if Drawing is unavailable.
    ...
    -- ESP Tracer: thicker Drawing line + thicker Beam fallback.
...
local function createStealBar()
    -- Adapt original floating Steal bar visual; K7 steal backend preserved.
    pcall(function()
    ...
    -- Same size/layout as the original Adapt floating bar.
    ...
    -- Adapt-style draggable behavior, with K7 position persistence kept.
    do
    ...
    -- Compatibility with the existing K7 steal engine.
...
local function buildGui()
    ...
    -- Preserve Adapt window position/scroll when the visual theme rebuilds.
    ...
    -- ADAPT visual palette mapped onto K7's saved theme names.
    ...
    local function rebuildForTheme()
        ...
        -- Keep the overhead Speed color synced with the selected hub theme.
    ...
    makeInputRow("FOV Customize",fovValue,function(v,box)
        local n=tonumber(v)
    ...
    -- Sync visual setters with live state.
    ...
    -- GLOBAL KEYBIND HANDLER (K7 backend)
    trackConn(UIS.InputBegan:Connect(function(input,gpe)
        ...
        elseif kbMatch(KB.TPFloor,kc) then
            -- TP Down has priority if an old config accidentally saved the same key as Drop.
            runTPFloor()
...
local _okInit, _errInit = pcall(function()
    loadConfigKeys()
    -- When intro is enabled, all Spirit Hub UI stays hidden until the intro ends or is skipped.
...
-- SPIRIT HUB CUSTOM INTRO OVERRIDE (user supplied)
playIntroAnimation = function()
    if introEnabled == false then return end
    local introSource = [====[
...
-- END SPIRIT HUB CUSTOM INTRO OVERRIDE

-- Intro first, Spirit Hub second. The hub stays disabled until the intro finishes or is skipped.
